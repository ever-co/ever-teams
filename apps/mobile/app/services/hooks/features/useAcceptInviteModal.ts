...
export function useAcceptInviteModal() {
	...
	useEffect(() => {
		if (isSuccess) {
			...
			const latestInvitationsSorted = latestInvitations.slice().sort((a, b) => a.id.localeCompare(b.id));

			const myInvitationsRefSorted = myInvitationsRef.current.slice().sort((a, b) => a.id.localeCompare(b.id));
