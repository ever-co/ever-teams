...
export function useTeamMemberCard(member: IOrganizationTeamList['members'][number] | undefined) {
	const {
		TaskStore: { activeTaskId, setActiveTaskId, setActiveTask, activeTask, teamTasks: tasks, tasksStatisticsState },
	...
	memberTaskRef.current = useMemo(() => {
		...
		if (responseTask) {
			...
			const taskStatistics =
				all && Array.isArray(all) ? all.find((statistics) => statistics.id === responseTask.id) : null;

			responseTask.totalWorkedTime =
				taskStatistics && typeof taskStatistics === 'object' ? taskStatistics.duration || 0 : 0;
		}
		return responseTask;
