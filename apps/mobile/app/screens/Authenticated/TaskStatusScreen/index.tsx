...
export const TaskStatusScreen: FC<AuthenticatedDrawerScreenProps<'TaskStatus'>> = function AuthenticatedDrawerScreen(
  ...
  const [desiredSnapIndex, setDesiredSnapIndex] = useState(1); // Track desired snap index

  // ref
  const sheetRef = useRef<BottomSheet>(null);

  // Define snap points like in the TaskVersionScreen - use multiple snap points
  const snapPoints = useMemo(() => ['50%', '85%', '100%'], []);

  // Using the same approach as TaskVersionScreen
  const openForEdit = useCallback((item: ITaskStatusItem) => {
    setEditMode(true);
    setItemToEdit(item);
    setBottomSheetVisible(true);

    setDesiredSnapIndex(1);
  ...
  const handleCreateNew = useCallback(() => {
    ...
    setDesiredSnapIndex(1); // Use 50% height for new item
  ...
  useEffect(() => {
    const keyboardWillShowListener = Keyboard.addListener(
      ...
      (e) => {
        ...
        if (bottomSheetVisible && sheetRef.current) {
          setDesiredSnapIndex(2); // Use higher snap point (70%) for keyboard
          // Force immediate update of the sheet position
          setTimeout(() => {
            if (sheetRef.current) {
