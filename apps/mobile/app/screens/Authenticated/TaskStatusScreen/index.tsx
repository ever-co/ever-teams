...
export const TaskStatusScreen: FC<AuthenticatedDrawerScreenProps<"TaskStatus">> = function AuthenticatedDrawerScreen(_props) {
            ...
            <Animated.View style={{ opacity: Animated.add(0.1, Animated.multiply(fall, 1.0)) }}>
                <View style={[$headerContainer, { backgroundColor: colors.background }]}>
                    <View style={[styles.container, { backgroundColor: colors.background }]}>
                        ...
                        <Text style={[styles.title, { color: colors.primary }]}>{translate("settingScreen.statusScreen.mainTitle")}</Text>
                    </View>
                </View>
                <ScrollView style={{ width: "100%", padding: 20, maxHeight: "80%" }} bounces={false}>
                    ...
                    <View style={{ minHeight: 200, justifyContent: "center", alignItems: "center" }}>
                        ...
                        {statuses?.items?.map((item, index) => (
                            <View  key={index}  style={{ marginBottom: statuses?.items.length === index + 1 ? 40 : 0, width: "100%" }}>
                                <StatusItem
                ...
                <TouchableOpacity style={{ ...styles.createButton, borderColor: dark ? "#6755C9" : "#3826A6" }} onPress={() => {
                    ...
                    <Text style={{ ...styles.btnText, color: dark ? "#6755C9" : "#3826A6" }}>{translate("settingScreen.statusScreen.createStatusButton")}</Text>
                </TouchableOpacity>
            </Animated.View>
            <BottomSheet
                ref={sheetRef}
