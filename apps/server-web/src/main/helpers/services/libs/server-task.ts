...
export abstract class ServerTask {
  ...
  protected constructor(
    ...
    this.loggerObserver = new Observer((msg: string) => {
      ...
      const logType = this.isErrorMessage(msg)
        ? LOG_TYPES.SERVER_LOG_ERROR
        : LOG_TYPES.SERVER_LOG;
  ...
  protected async runTask(signal: AbortSignal): Promise<void> {
    ...
    return new Promise<void>((resolve, reject) => {
      try {
        ...
        const service = ChildProcessFactory.createProcess(
          this.processPath,
          this.args,
          signal,
        );
        ...
        this.config.setting = {
          server: { ...this.config.setting.server, [this.pid]: service.pid },
        };
      } catch (error) {
  ...
  public kill(callHandleError = true): void {
    ...
    try {
      if (this.pid && this.config.setting.server[this.pid]) {
        ...
        this.loggerObserver.notify(
          `[${this.pid.toUpperCase()}-${this.config.setting.server[this.pid]}]: stopped`,
        );
      }
    } catch (error: any) {
      if (callHandleError) {
