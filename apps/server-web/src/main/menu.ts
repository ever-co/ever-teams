...
export default class MenuBuilder {
  ...
  initialMenu(): AppMenu[] {
    ...
    return [
        ...
        submenu: [
          {
            ...
            click: () => {
              this.eventEmitter.emit(EventLists.OPEN_WINDOW, {
                windowType: WindowTypes.ABOUT_WINDOW,
              });
            },
          },
          { type: 'separator' },
      ...
      },
      {
        ...
        submenu: [
          {
            label: 'MENU_APP.APP_SUBMENU.APP_UNDO',
            accelerator: 'CmdOrCtrl+Z',
            role: 'undo',
          },
          {
            label: 'MENU_APP.APP_SUBMENU.APP_REDO',
            accelerator: 'Shift+CmdOrCtrl+Z',
            role: 'redo',
          },
          { type: 'separator' },
          {
            label: 'MENU_APP.APP_SUBMENU.APP_CUT',
            accelerator: 'CmdOrCtrl+X',
            role: 'cut',
          },
          {
            label: 'MENU_APP.APP_SUBMENU.APP_COPY',
            accelerator: 'CmdOrCtrl+C',
            role: 'copy',
          },
          {
            label: 'MENU_APP.APP_SUBMENU.APP_PASTE',
            accelerator: 'CmdOrCtrl+V',
            role: 'paste',
          },
          {
            label: 'MENU_APP.APP_SUBMENU.APP_SELECT_ALL',
      ...
      },
      {
        ...
        submenu: [
          ...
          },
          {
            ...
            click() {
              shell.openExternal(config.COMPANY_GITHUB_LINK);
            },
          },
        ],
      },
  ...
  updateAppMenu(
    menuItem: string,
    context: { label?: string; enabled?: boolean },
    contextMenuItems: any,
    i18nextMainBackend: typeof i18n,
  ) {
    const menuIdx: number = contextMenuItems.findIndex(
      (item: any) => item.id === menuItem,
    );
    if (menuIdx > -1) {
      ...
      Menu.setApplicationMenu(
        Menu.buildFromTemplate(
          this.translateAppMenu(i18nextMainBackend, newMenu),
        ),
      );
    } else {
      const newMenu = [...contextMenuItems];
  ...
  buildTemplateMenu(windowType: IWindowTypes, i18nextMainBackend: typeof i18n) {
    ...
    return Menu.buildFromTemplate(
      this.translateAppMenu(i18nextMainBackend, menu),
    );
  }
}
