...
const createWindow = async (
  ...
  return windowFactory.buildWindow({
    windowType,
    menu: appMenu.buildTemplateMenu(windowType, i18nextMainBackend),
  });
};
...
const onInitApplication = () => {
  ...
  i18nextMainBackend.on(
    'languageChanged',
    debounce((lng) => {
      if (i18nextMainBackend.isInitialized && storeConfig?.general?.setup) {
        trayMenuItems = trayMenuItems.length
          ? trayMenuItems
          : defaultTrayMenuItem(eventEmitter);
        updateTrayMenu(
          'none',
          {},
          eventEmitter,
        ...
        Menu.setApplicationMenu(
          appMenu.buildTemplateMenu(WindowTypes.LOG_WINDOW, i18nextMainBackend),
        );
      } else {
        Menu.setApplicationMenu(
          appMenu.buildTemplateMenu(
            WindowTypes.SETUP_WINDOW,
            i18nextMainBackend,
          ),
        );
      }
  ...
  eventEmitter.on(EventLists.webServerStarted, () => {
    ...
    updateTrayMenu(
      'SERVER_STATUS',
      { label: 'MENU.SERVER_STATUS_STARTED' },
      eventEmitter,
      tray,
      trayMenuItems,
      i18nextMainBackend,
  ...
  eventEmitter.on(EventLists.gotoSetting, async () => {
    ...
    settingWindow?.webContents.once('did-finish-load', () => {
      setTimeout(() => {
        ...
        SendMessageToSettingWindow(
          SettingPageTypeMessage.loadSetting,
          serverSetting,
        );
      }, 50);
    });
  ...
  eventEmitter.on(EventLists.UPDATE_ERROR, (data) => {
    ...
    SendMessageToSettingWindow(SettingPageTypeMessage.updateError, {
  ...
  eventEmitter.on(EventLists.UPDATE_PROGRESS, (data) => {
    ...
    SendMessageToSettingWindow(SettingPageTypeMessage.downloadingUpdate, {
      percent: Math.floor(data.percent || 0),
    });
  });
  ...
  eventEmitter.on(EventLists.CHANGE_THEME, (data) => {
    ...
    settingWindow?.webContents.send('themeSignal', {
      type: SettingPageTypeMessage.themeChange,
      data,
    });
    setupWindow?.webContents.send('themeSignal', {
      type: SettingPageTypeMessage.themeChange,
      data,
    });
  });
...
const getWebDirPath = () => {
  ...
  const packDirFilesPath = path.join(
    process.resourcesPath,
    'release',
    'app',
    'dist',
...
ipcMain.on(IPC_TYPES.SETTING_PAGE, async (event, arg) => {
  switch (arg.type) {
    ...
    case SettingPageTypeMessage.updateSetting:
      ...
      createIntervalAutoUpdate();
      event.sender.send(IPC_TYPES.UPDATER_PAGE, {
        type: SettingPageTypeMessage.updateSettingResponse,
        data: true,
      });
      break;
