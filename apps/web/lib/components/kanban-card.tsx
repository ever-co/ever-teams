...
export function Priority({ level }: { level: ITaskPriority }) {
  ...
  const levelIntoNumber =
    levelSmallCase === 'low'
      ? 1
      : levelSmallCase === 'medium'
      ? 2
      : levelSmallCase === 'high'
      ? 3
      : 4;
  const numberArray = Array.from(
    { length: levelIntoNumber },
    (_, index) => index + 1
  );
...
export default function Item(props: ItemProps) {
  ...
  activeTeam?.members?.forEach((member) => {
    const totalWorkedTasks =
      member?.totalWorkedTasks?.find((i) => i.id === item?.id) || null;
    if (totalWorkedTasks) {
  ...
  const taskAssignee: ImageOverlapperProps[] = item.members.map(
    (member: any) => {
      return {
        id: member.user.id,
  ...
  const currentMember = activeTeam?.members.find(
    (member) => member.id === memberInfo.member?.id || item?.id
  );
            ...
            {activeTask?.id == item.id ? (
              ...
              <Link href={`/task/${item.id}`}>
                <div className="w-64 relative overflow-hidden">
                  ...
                  <span className="text-grey text-normal mx-1">
                    #{item.number}
                  </span>
                  {item.title}
