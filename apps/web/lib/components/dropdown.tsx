...
import { IClassName } from '@app/interfaces';
import { clsxm } from '@app/utils';
import { Listbox, ListboxOption, Popover, Transition } from '@headlessui/react';
import { ChevronDownIcon } from '@heroicons/react/20/solid';
import React, { Dispatch, PropsWithChildren, SetStateAction } from 'react';
import { Card } from './card';
import { SpinnerLoader } from './loader';
...
export function Dropdown<T extends DropdownItem>({
	...
	const [open, setOpen] = React.useState(false);
	return (
		<div className={clsxm('relative rounded-xl', className)}>
			{open && <div onClick={() => setOpen(false)} className="fixed inset-0 bg-transparent"></div>}
			<Listbox
				as="div"
				value={Value}
				onChange={onChange}
				className={clsxm('rounded-xl', className)}
				disabled={publicTeam}
			>
				<Listbox.Button
					className={clsxm(
						'input-border',
						'w-full flex justify-between rounded-xl px-3 py-2 text-sm items-center',
						'font-normal outline-none bg-white',
						buttonClassName
					)}
					onClick={() => setOpen(!open)}
					style={buttonStyle}
