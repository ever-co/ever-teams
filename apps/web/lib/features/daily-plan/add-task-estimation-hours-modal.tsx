...
import { useDailyPlan, useModal, useTeamTasks, useTimerView } from '@app/hooks';
import { TaskNameInfoDisplay } from '../task/task-displays';
import { TaskEstimate } from '../task/task-estimate';
import { IDailyPlan, ITeamTask } from '@app/interfaces';
import { EnforcePlanedTaskModal } from './enforce-planed-task-modal';
...
export function AddTasksEstimationHoursModal(props: IAddTasksEstimationHoursModalProps) {
	...
	const {
		isOpen: isEnforcePlannedTaskModalOpen,
		openModal: openEnforcePlannedTaskModal,
		closeModal: closeEnforcePlannedTaskModal
	} = useModal();
	...
	const doesTaskExistInTodayPlan = plan.tasks?.find((task) => task.id === activeTeamTask?.id);
	...
	const handleCloseEnforcePlannedTaskModal = () => {
		closeEnforcePlannedTaskModal();
		startTimer();
	};
		...
		<>
			...
			{activeTeamTask && (
				<EnforcePlanedTaskModal
					task={activeTeamTask}
					plan={plan}
					open={isEnforcePlannedTaskModalOpen}
					closeModal={() => {
						handleCloseEnforcePlannedTaskModal();
					}}
					content={`Would you like to add the task ${activeTeamTask?.number} to Today's plan?`}
				/>
			)}
		</>
	);
