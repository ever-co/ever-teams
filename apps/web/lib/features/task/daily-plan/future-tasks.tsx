...
export function FutureTasks({ profile }: { profile: any }) {
  ...
  const { setDate, date } = useDateRange(
    window.localStorage.getItem('daily-plan-tab')
  );
  const [futureDailyPlanTasks, setFutureDailyPlanTasks] = useState<
    IDailyPlan[]
  >(futurePlans);
  useEffect(() => {
      ...
      {futureDailyPlanTasks?.length > 0 ? (
        <DragDropContext
          onDragEnd={(result) =>
            handleDragAndDrop(
              result,
              futureDailyPlanTasks,
              setFutureDailyPlanTasks
            )
          }
        >
          <Accordion
