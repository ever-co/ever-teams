...
} from '../../components/image-overlapper';
...
export default function TaskBlockCard(props: TaskItemProps) {
  ...
  activeTeam?.members?.forEach((member) => {
    const totalWorkedTasks =
      member?.totalWorkedTasks?.find((i) => i.id === task?.id) || null;
    if (totalWorkedTasks) {
  ...
  const taskAssignee: ImageOverlapperProps[] = task.members?.map(
    (member: any) => {
      return {
        id: member.user?.id,
  ...
  const currentMember = activeTeam?.members.find(
    (member) => member.id === memberInfo.member?.id || task?.id
  );
