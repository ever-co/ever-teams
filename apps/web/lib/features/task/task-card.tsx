...
export function TaskCard(props: Props) {
  ...
  const members = useMemo(() => activeTeam?.members || [], [
    activeTeam?.members
  ]);
  ...
  const activeMembers = useMemo(
    () => task != null && task?.members?.length > 0,
    [task]
  );
  const hasMembers = useMemo(() => task?.members && task?.members?.length > 0, [
    task?.members
  ]);
          ...
          <div className="w-[20%] flex justify-around items-center">
            ...
            <ImageComponent
              ...
              hasInfo={
                !hasMembers
                  ? 'Assign this task'
                  : 'Assign this task to more people'
              }
            />
          </div>
        )}
...
function TimerButtonCall({
  ...
  const requirePlan = useMemo(() => activeTeam?.requirePlanToTrack, [
  ...
  return loading ? (
    ...
    <>
      <TimerButton
        ...
        disabled={
          activeTaskStatus ? disabled : task.status === 'closed' || !canTrack
        }
...
function TaskInfo({
  ...
  return (
    ...
    >
      ...
      {/* Task status */}
      {task && (
        <TaskAllStatusTypes task={task} tab={tab} dayPlanTab={dayPlanTab} />
      )}
      {!task && <div className="self-center py-1 text-center">--</div>}
    </div>
  );
...
function TaskCardMenu({
  ...
  const isTaskPlannedMultipleTimes =
    allPlans.reduce((count, plan) => {
      if (plan?.tasks) {
        const taskCount = plan.tasks.filter((_task) => _task.id === task.id)
          .length;
...
export function AddTaskToPlanComponent({
  task,
  employee
}: {
  task: ITeamTask;
...
export function RemoveManyTaskFromPlan({
  task,
  member
}: {
  task: ITeamTask;
