...
export const KanbanView = ({
  ...
  const reorderKanbanTasks = ({
    ...
    if (sourceDroppableID === destinationDroppableID) {
      const reorderedKanbanTasks = reorderTask(
        currentTaskStatus,
        sourceIndex,
        destinationIndex
      );
    ...
    const updateTaskStatusData = {
      ...targetStatus,
      status: taskstatus,
      taskStatusId: ts.find(
        (v) => v.name?.toLowerCase() == taskstatus.toLowerCase()
      )?.id
    };
  ...
  const onDragEnd = (result: DropResult) => {
    ...
    if (result.type === 'COLUMN') {
      const reorderedItem = reorderColumn(
        columns,
        source.index,
        destination.index
      );
