...
export function UserProfilePlans() {
  ...
  const [currentOutstanding, setCurrentOutstanding] = useLocalStorageState<
    FilterOutstanding
  >('outstanding', 'ALL');

  const [currentTab, setCurrentTab] = useLocalStorageState<FilterTabs>(
    'daily-plan-tab',
    'Today Tasks'
  );

  const [currentDataDailyPlan, setCurrentDataDailyPlan] = useAtom(
    dataDailyPlanState
  );
  ...
  const [filterFuturePlanData, setFilterFuturePlanData] = useState<
    IDailyPlan[]
  >(futurePlans);
  const [filterPastPlanData, setFilteredPastPlanData] = useState<IDailyPlan[]>(
    pastPlans
  );
  const [filterAllPlanData, setFilterAllPlanData] = useState<IDailyPlan[]>(
    sortedPlans
  );
                    ...
                    >
                      ...
                      >
                        {t(
                          `task.tabFilter.${filter
                            ...
                            .replace(
                              ' ',
                              '_'
                            )}` as DottedLanguageObjectStringPaths
                        )}
                        <span
                          className={clsxm(
                        ...
                        >
                          ...
                          {filter === 'Future Tasks' &&
                            getTotalTasks(filterFuturePlanData)}
                          {filter === 'Past Tasks' &&
                            getTotalTasks(filterPastPlanData)}
                          {filter === 'All Tasks' &&
                            getTotalTasks(filterAllPlanData)}
                          {filter === 'Outstanding' &&
                            estimatedTotalTime(
  ...
  return (
    <div className="flex flex-col gap-6">
      {Array.isArray(plans) && plans?.length > 0 ? (
        <DragDropContext
          onDragEnd={(result) => handleDragAndDrop(result, plans, setPlans)}
        >
          <Accordion
            type="multiple"
            className="text-sm"
            defaultValue={
              currentTab === 'Today Tasks'
                ...
                : [
                    plans?.map(
                      (plan) => new Date(plan.date).toISOString().split('T')[0]
                    )[0]
                  ]
            }
          >
                          ...
                          <>
                            {canSeeActivity ? (
                              <div className="flex justify-end">
                                <AlertPopup
                                  open={
                                    currentDeleteIndex === index && popupOpen
                                  }
                                  buttonOpen={
                                    //button open popup
                                    <Button
...
export function PlanHeader({
  ...
  const estimatedTime = useMemo(
    () => (plan.tasks ? sumTimes(plan.tasks, 'estimate') : 0),
    [plan.tasks]
  );
  const totalWorkTime = useMemo(
    () => (plan.tasks ? sumTimes(plan.tasks, 'totalWorkedTime') : 0),
    [plan.tasks]
  );
  ...
  const readyTasks = useMemo(
    () => plan.tasks?.filter((task) => task.status === 'ready').length ?? 0,
    [plan.tasks]
  );
  ...
  const completionPercent =
    totalTasks > 0 ? ((completedTasks * 100) / totalTasks).toFixed(0) : '0.0';
