...
import { IActivityReportGroupByDate, IActivityItem } from '@/core/types/interfaces/activity/activity-report';
import {
	SortableColumn,
	TableColumn,
	createDateCompare,
...
export const useTableConfig = <T extends Record<string, any>>(options: TableConfigOptions<T>): TableConfig<T> => {
	...
	return useTableConfig<IActivityReportGroupByDate>({
		columnDefinitions: [
			...
			},
			{
				...
				getValue: (data) => {
					const activities =
						data.employees?.flatMap((emp) => emp.projects.flatMap((proj) => proj.activity)) || [];
					const totalPercentage = activities.reduce(
						(sum, act) => sum + parseFloat(act.duration_percentage),
						0
					);
