...
export function useTimeLogs() {
  ...
  const [timerLogsDailyReport, setTimerLogsDailyReport] = useAtom(
    timerLogsDailyReportState
  );

  const {
    loading: timerLogsDailyReportLoading,
    queryCall: queryTimerLogsDailyReport
  } = useQuery(getTimerLogsDailyReportRequestAPI);
