...
export function useFilterDateRange(
  itemsDailyPlan: IDailyPlan[],
  typeItems?: 'future' | 'past' | 'all'
) {
  ...
  const [originalAllPlanData, setOriginalAllPlanState] = useAtom(
    originalAllPlanState
  );
  const [originalPastPlanData, setOriginalPastPlanData] = useAtom(
    originalPastPlanDataState
  );
  const [originalFuturePlanData, setOriginalFuturePlanData] = useAtom(
    originalFuturePlanState
  );
  ...
  const updateOriginalPlanData = useMemo(
    ...
    [
      typeItems,
      setOriginalAllPlanState,
      setOriginalFuturePlanData,
      setOriginalPastPlanData
    ]
  );
