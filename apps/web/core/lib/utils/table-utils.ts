...
export const createNumericCompare =
	() =>
...
export const createStringCompare =
	() =>
	(a: string | number | null | undefined, b: string | number | null | undefined): number => {
		// Handle null/undefined values
		if (a == null && b == null) return 0;
		if (a == null) return -1;
		if (b == null) return 1;
...
export const createDateCompare =
	() =>
...
export const aggregateNumericValues = <T>(data: T[], getValue: (item: T) => number): number => {
	return data.reduce((sum, item) => sum + getValue(item), 0);
};

export const countUniqueValues = <T>(data: T[], getValue: (item: T) => string | number): number => {
...
export const safeArrayForEach = <T>(arr: T[] | undefined | null, callback: (item: T) => void): void => {
