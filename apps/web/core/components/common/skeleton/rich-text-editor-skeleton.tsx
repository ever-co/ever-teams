		...
		<div className={clsxm('flex flex-col prose dark:prose-invert', className)}>
			...
			<div className="flex items-center gap-2 p-2 border-b border-gray-200 dark:border-gray-700">
				...
				{[...Array(6)].map((_, i) => (
					<div
						key={i}
						className="w-8 h-8 bg-[#F0F0F0] dark:bg-[#353741] animate-pulse rounded"
					/>
				))}
			...
			<div className="h-64 p-4 space-y-3 bg-transparent">
 ...
 */
export const TaskDetailsAsideSkeleton: FC<{ className?: string }> = ({ className }) => {
	return (
		<div className={clsxm(
			...
			{[...Array(5)].map((_, i) => (
				<div key={i}>
					...
					{i < 4 && (
						<div className="h-[0.0625rem] bg-[#F2F2F2] dark:bg-[#26272C] w-[95%] mx-auto" />
					)}
				</div>
			))}
		</div>
 ...
 */
export const IssueCardSkeleton: FC<{ className?: string; title?: string }> = ({ 
	className, 
	title = "Issues" 
}) => {
	return (
		<div className={clsxm(
			...
			<div className="py-4 space-y-3">
				{[...Array(2)].map((_, i) => (
					<div key={i} className="flex items-center gap-3 p-3 bg-white dark:bg-dark--theme rounded-lg">
						<div className="w-6 h-6 bg-[#F0F0F0] dark:bg-[#353741] animate-pulse rounded" />
						<div className="flex-1 space-y-1">
