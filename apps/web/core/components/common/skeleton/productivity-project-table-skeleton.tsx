...
export const ProductivityProjectTableSkeleton: FC<ProductivityProjectTableSkeletonProps> = ({ className }) => {
	...
	return (
		<Card
			className={`bg-white rounded-md border border-gray-100 dark:border-gray-800 dark:bg-dark--theme-light min-h-[600px] w-full ${className || ''}`}
		>
			<Table>
				...
				<TableBody>
					...
					{[1, 2].map((projectIndex) => (
						<Fragment key={`project-group-${projectIndex}`}>
							{/* Project Header Row */}
							...
							{[1, 2].map((dateIndex) => (
								<Fragment key={`date-group-${dateIndex}`}>
									{/* Date Header Row */}
									<TableRow
										key={`date-header-${projectIndex}-${dateIndex}`}
										className="bg-gray-50/30 dark:bg-gray-800"
									>
										<TableCell>
