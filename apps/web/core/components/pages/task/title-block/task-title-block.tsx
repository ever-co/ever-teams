		...
		<div className="flex flex-col gap-[1.1875rem]" ref={titleContainerRef}>
			...
			<div className="flex flex-col items-start">
				<div className="flex flex-row gap-3 justify-start items-center">
					...
					{task?.issueType !== TaskTypeEnum.EPIC && task && (
						<div className="flex gap-3 items-center">
							{/* Current Issue Type is Task|Bug and Parent Issue is Not an Epic */}
							{(!task?.issueType ||
								task?.issueType === TaskTypeEnum.TASK ||
								task?.issueType === TaskTypeEnum.BUG) &&
								task?.rootEpic &&
		...
		<HoverCard>
			<HoverCardTrigger asChild>
				<Link
					...
					target="_blank"
					className={clsxm(
						task.parent.issueType === TaskTypeEnum.EPIC && 'bg-[#8154BA]',
						task.parent.issueType === TaskTypeEnum.STORY && 'bg-[#54BA951A]',
						task.parent.issueType === TaskTypeEnum.BUG && 'bg-[#C24A4A1A]',
						(task.parent.issueType === TaskTypeEnum.TASK || !task.parent.issueType) && 'bg-[#5483ba]',
						'rounded-[0.1875rem] text-center !h-7 3xl:h-6 flex justify-center items-center py-[0.25rem] px-2.5'
					)}
				>
					<span
						className={clsxm(
							task.parent.issueType === TaskTypeEnum.EPIC && 'text-white',
							task.parent.issueType === TaskTypeEnum.STORY && 'text-[#27AE60]',
							task.parent.issueType === TaskTypeEnum.BUG && 'text-[#C24A4A]',
							(task.parent.issueType === TaskTypeEnum.TASK || !task.parent.issueType) && 'text-white',
							'font-medium text-[0.5rem] 3xl:text-xs max-w-[10rem] overflow-hidden text-ellipsis whitespace-nowrap'
						)}
					>
						<span
							className={clsxm(
								task.parent.issueType === TaskTypeEnum.EPIC && 'text-[#FFFFFF80]',
								task.parent.issueType === TaskTypeEnum.STORY && 'text-[#27AE6080]',
								task.parent.issueType === TaskTypeEnum.BUG && 'text-[#C24A4A80]',
								(task.parent.issueType === TaskTypeEnum.TASK || !task.parent.issueType) && 'text-white'
							)}
...
const ParentTaskInput = ({ task }: { task: ITask | null }) => {
	...
	return task && task.issueType !== TaskTypeEnum.EPIC ? (
