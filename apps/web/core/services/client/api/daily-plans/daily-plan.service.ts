...
class DailyPlanService extends APIService {
	...
	getAllDayPlans = async (): Promise<PaginationResponse<TDailyPlan>> => {
		try {
			...
			return validatePaginationResponseSmart(dailyPlanSchema, response.data, 'getAllDayPlans API response');
		} catch (error) {
	...
	getMyDailyPlans = async (): Promise<PaginationResponse<TDailyPlan>> => {
		try {
			...
			// Validate the response data using smart validation with auto-normalization
			return validatePaginationResponseSmart(dailyPlanSchema, response.data, 'getMyDailyPlans API response');
		} catch (error) {
			if (error instanceof ZodValidationError) {
				this.logger.error(
	...
	getDayPlansByEmployee = async ({ employeeId }: { employeeId: string }): Promise<PaginationResponse<TDailyPlan>> => {
		try {
			...
			// Validate the response data using smart validation with auto-normalization
			return validatePaginationResponseSmart(dailyPlanSchema, response.data, 'getDayPlansByEmployee API response');
		} catch (error) {
			if (error instanceof ZodValidationError) {
				this.logger.error(
	...
	getPlansByTask = async ({ taskId }: { taskId: string }): Promise<PaginationResponse<TDailyPlan>> => {
		try {
			...
			// Validate the response data using smart validation with auto-normalization
			return validatePaginationResponseSmart(dailyPlanSchema, response.data, 'getPlansByTask API response');
		} catch (error) {
			if (error instanceof ZodValidationError) {
				this.logger.error(
	...
	createDailyPlan = async (data: TCreateDailyPlan): Promise<TDailyPlan> => {
		try {
			...
			return validateApiResponseSmart(dailyPlanSchema, response.data, 'createDailyPlan API response');
		} catch (error) {
			if (error instanceof ZodValidationError) {
				this.logger.error(
	...
	updateDailyPlan = async (data: TUpdateDailyPlan, planId: string): Promise<TDailyPlan> => {
		try {
			// Validate input data before sending
			const validatedInput = validateApiResponseSmart(updateDailyPlanSchema, data, 'updateDailyPlan input data');
			...
			// Validate the response data
			return validateApiResponseSmart(dailyPlanSchema, response.data, 'updateDailyPlan API response');
		} catch (error) {
			if (error instanceof ZodValidationError) {
				this.logger.error(
	...
	addTaskToPlan = async (data: TDailyPlanTasksUpdate, planId: string): Promise<TDailyPlan> => {
		try {
			...
			// Validate the response data
			return validateApiResponseSmart(dailyPlanSchema, response.data, 'addTaskToPlan API response');
		} catch (error) {
			if (error instanceof ZodValidationError) {
				this.logger.error(
	...
	removeTaskFromPlan = async (data: TRemoveTaskFromPlansRequest, planId: string): Promise<TDailyPlan> => {
		try {
			...
			// Validate the response data
			return validateApiResponseSmart(dailyPlanSchema, response.data, 'removeTaskFromPlan API response');
		} catch (error) {
			if (error instanceof ZodValidationError) {
				this.logger.error(
	...
	removeManyTaskFromPlans = async ({
		...
		try {
			...
			const validatedPlans = response.data.map((plan, index) =>
				validateApiResponseSmart(dailyPlanSchema, plan, `removeManyTaskFromPlans API response item ${index}`)
			);
