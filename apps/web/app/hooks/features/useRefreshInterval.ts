...
export function useRefreshIntervalV2(
  callback: any,
  delay: number,
  ...params: any[]
) {
  ...
  useEffect(() => {
    try {
      if (typeof window !== 'undefined') {
        setDataSync(
          JSON.parse(window.localStorage.getItem('conf-is-data-sync') || 'true')
        );
        setDataSyncMode(
          (window.localStorage.getItem('conf-data-sync-mode') as ESyncMode) ??
            'PULL'
        );
      }
    } catch (error) {
