...
export const useEmployee = () => {
  ...
  const [workingEmployees, setWorkingEmployees] = useAtom(
    workingEmployeesState
  );
  const [workingEmployeesEmail, setWorkingEmployeesEmail] = useAtom(
    workingEmployeesEmailState
  );
    ...
    getWorkingEmployeeQueryCall(
      ...
      if (data?.items && data?.items?.length) {
        ...
        setWorkingEmployeesEmail(
          items.map((item: any) => item.user?.email || '')
        );
      }
    });
  }, [
...
export const useEmployeeUpdate = () => {
  const { queryCall: employeeUpdateQuery, loading: isLoading } = useQuery(
    updateEmployeeAPI
  );
