...
function useLocalTimeCounter(
  ...
  const [timeCounterInterval, setTimeCounterInterval] = useAtom(
    timeCounterIntervalState
  );
  const [localTimerStatus, setLocalTimerStatus] = useAtom(
    localTimerStatusState
  );
...
export function useTimer() {
  ...
  const [timerStatusFetching, setTimerStatusFetching] = useAtom(
    timerStatusFetchingState
  );
  ...
  const { queryCall: stopTimerQueryCall, loading: stopTimerLoading } = useQuery(
    stopTimerAPI
  );
  const {
  ...
  const hasPlanForTomorrow = myDailyPlans.items.find(
    (plan) =>
      moment(plan.date).format('YYYY-MM-DD') === tomorrow.format('YYYY-MM-DD')
  );
  ...
  const getTimerStatus = useCallback(
    (deepCheck?: boolean) => {
      ...
      return queryCall(user?.tenantId, user?.employee.organizationId).then(
        (res) => {
  ...
  useEffect(() => {
    ...
    const canStop =
      lastActiveTaskId.current !== null && taskId !== lastActiveTaskId.current;
