...
export interface UseReportActivityProps extends Omit<ITimeLogReportDailyChartProps, 'logType' | 'activityLevel' | 'start' | 'end'> {
    logType?: TimeLogType[];
    activityLevel: {
        start: number;
        end: number;
...
const defaultProps: Required<Pick<UseReportActivityProps, 'startDate' | 'endDate' | 'groupBy' | 'activityLevel' | 'logType' | 'start' | 'end'>> = {
...
export function useReportActivity() {
    ...
    const fetchStatisticsCounts = useCallback(async (customProps?: Partial<UseReportActivityProps>) => {
        if (!user || !getMergedProps) {
            return;
        }
        try {
