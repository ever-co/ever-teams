...
export function useKanban() {
  ...
  useEffect(() => {
    if (!taskStatusHook.loading && !tasksFetching) {
      ...
      const tasks = newTask
        ...
        .filter((task: ITeamTask) => {
          return labels.length
            ? labels.some((label) =>
                task.tags.some((tag) => tag.name === label)
              )
            : true;
        })
        .filter((task: ITeamTask) => {
      ...
      taskStatusHook.taskStatus.map((taskStatus: ITaskStatusItemList) => {
        kanban = {
          ...kanban,
          [taskStatus.name ? taskStatus.name : '']: getTasksByStatus(
            taskStatus.id
          )
        };
      });
  ...
  const toggleColumn = (column: string, status: boolean) => {
    const columnData = taskStatusHook.taskStatus.filter(
      (taskStatus: ITaskStatusItemList) => {
  ...
  const isColumnCollapse = (column: string) => {
    const columnData = taskStatusHook.taskStatus.find(
      (taskStatus: ITaskStatusItemList) => {
        return taskStatus.name === column;
      }
    );
