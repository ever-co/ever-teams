...
export function useOrganizationTeams() {
  ...
  const [isTeamMemberJustDeleted, setIsTeamMemberJustDeleted] = useAtom(
    isTeamMemberJustDeletedState
  );
  ...
  const {
    loading: deleteOrganizationTeamLoading,
  ...
  const loadTeamsData = useCallback(() => {
    ...
    return queryCall(
      ...
      if (
        !latestTeams.find((team: any) => team.id === teamId) &&
        latestTeams.length
      ) {
        setIsTeamMemberJustDeleted(true);
      ...
      teamId &&
        ...
        queryCallTeam(
          ...
          if (!isEqual(latestTeamsSorted, teamsRefSorted)) {
            setTeams([
              newTeam,
              ...latestTeams.filter((team: any) => team.id !== newTeam.id)
            ]);
