...
export const activeTeamDailyPlansState = atom<PaginationResponse<IDailyPlan>>({
	key: 'activeTeamDailyPlansState',
	default: { items: [], total: 0 }
});

const createDailyPlanCountFilterAtom = (key: string | any) =>
	atom<number>({
		key,
		default: 0
	});

const createDailyPlanAtom = (key: string | any) =>
	atom<IDailyPlan[]>({
		key,
		default: []
	});

const createDateRangeAtom = (key: string | any) =>
	atom<DateRange | undefined>({
		key,
		default: { from: undefined, to: undefined }
	});

export const dataDailyPlanState = createDailyPlanAtom('originalPlanState');
const getFirstAndLastDateSelector = (key: string | any) =>
	selector({
		key,
		get: ({ get }) => {
...
const createFilteredDailyPlanDataSelector = (
	key: string | any,
	dateRangeState: RecoilState<DateRange | undefined>,
	originalDataState: RecoilState<IDailyPlan[]>
) =>
	selector({
		key,
		get: ({ get }) => {
...
export const setCreateFilteredDailyPlanDataSelector = () =>
	selector({
