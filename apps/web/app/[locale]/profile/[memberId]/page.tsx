...
const Profile = React.memo(function ProfilePage({
  params
}: {
  ...
  const profileIsAuthUser = useMemo(() => profile.isAuthUser, [
    profile.isAuthUser
  ]);
  ...
  return (
    <MainLayout
      showTimer={
        (headerSize <= 11.8 && isTrackingEnabled) || !profileIsAuthUser
      }
    >
      <ResizablePanelGroup direction="vertical">
        ...
        >
          <MainHeader
            fullWidth={fullWidth}
            className={cn(hookFilterType && ['pb-0'], 'pb-2')}
          >
            {/* Breadcrumb */}
