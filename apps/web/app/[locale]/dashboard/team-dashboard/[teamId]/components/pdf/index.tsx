...
const styles = StyleSheet.create({
  ...
  headerContainer: { alignItems: 'center', marginBottom: 20, borderBottom: 1, borderBottomColor: '#E5E7EB', paddingBottom: 16 },
  ...
  table: { width: '100%', borderStyle: 'solid', borderWidth: 1, borderRightWidth: 0, borderBottomWidth: 0, marginBottom: 16 },
  tableRow: { flexDirection: 'row', backgroundColor: '#ffffff', width: '100%', minHeight: 32 },
  tableHeader: { backgroundColor: '#F9FAFB' },
  tableCell: { width: '10%', borderStyle: 'solid', borderWidth: 1, borderLeftWidth: 0, borderTopWidth: 0, padding: '8 6' },
  memberCell: { width: '20%', borderStyle: 'solid', borderWidth: 1, borderLeftWidth: 0, borderTopWidth: 0, padding: '8 6' },
...
const getTaskDurationByType = (data: ITimerLogGrouped, type: string): number => {
  ...
  return employeeLog.tasks.reduce(
    (sum: number, task: { description: string; duration: number }) => (task.description?.toLowerCase().includes(type) ? sum + task.duration : sum),
    0
  ) || 0;
};
...
export function TeamStatsPDF({ rapportDailyActivity, title = 'Team Activity Report', startDate, endDate }: TeamStatsPDFProps) {
  const groupedByDate = rapportDailyActivity.reduce((acc: { [key: string]: ITimerLogGrouped[] }, curr: ITimerLogGrouped) => {
    const date = new Date(curr.date).toLocaleDateString('en-US', { weekday: 'long', day: '2-digit', month: 'short', year: 'numeric' });
              ...
              {activities.map((activity: ITimerLogGrouped, index: number) => (
                <View key={index} style={styles.tableRow}>
                  {TABLE_COLUMNS.map((column) => {
                    ...
                    const textStyle = column.textStyle === 'percentageText' ? styles.percentageText : styles.text;
                    return (
                      <View key={column.id} style={styles[column.style]}>
