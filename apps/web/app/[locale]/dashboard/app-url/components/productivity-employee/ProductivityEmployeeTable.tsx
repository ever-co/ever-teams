...
interface EmployeeActivityGroup {
  employee: IEmployee;
...
const ProjectLogo: React.FC = React.memo(() => (
  <div className="w-8 h-8 rounded-md bg-[#422AFB] flex items-center justify-center text-white text-xs">
    Ever
  </div>
));
...
const ActivityBar: React.FC<{ percentage: string; title: string }> = React.memo(({ percentage, title }) => (
  <TooltipProvider>
    <Tooltip>
      <TooltipTrigger asChild>
        <div className="flex flex-1 gap-2 items-center">
          <div className="overflow-hidden flex-1 h-2 bg-[#EDF2F7] rounded-full dark:bg-gray-700">
            <div
              className="h-full bg-[#422AFB] rounded-full"
      ...
      <TooltipContent>
        <p>{title}: {Math.round(parseFloat(percentage))}% of total time</p>
      </TooltipContent>
    </Tooltip>
  </TooltipProvider>
));
...
export const ProductivityEmployeeTable: React.FC<Props> = ({ data = [], isLoading }) => {
 ...
 const t = useTranslations();
  ...
  const groupedData = React.useMemo(() => {
    ...
    sortedData.forEach((dateGroup) => {
      if (!dateGroup?.date || !Array.isArray(dateGroup.employees)) {
        console.warn('Invalid date group format', dateGroup);
        return;
      }

      const date = dateGroup.date;

      dateGroup.employees.forEach((employeeData: any) => {
        const { employee, projects } = employeeData;

        if (!employee?.id) {
          console.warn('Invalid employee data: Missing ID', employee);
          return;
        }
        ...
        // Merge all activities from all projects for this date
      ...
      <Table>
        ...
        <TableBody>
          {employeeList.map((employeeGroup) => {
            ...
            const dates = Array.from(dateGroups.keys()).sort((a, b) =>
                ...
                <TableRow className="bg-gray-50/50 dark:bg-gray-800/50">
                  <TableCell colSpan={5}>
                    <div className="flex gap-2 items-center px-1 py-2">
                      ...
                      <span className="font-semibold text-gray-900 dark:text-gray-100">{employee.fullName}</span>
                      <span className="ml-auto text-sm text-gray-600 dark:text-gray-400">
                        Total time: {formatDuration(
                          Array.from(dateGroups.values())
                            .reduce((sum, group) => sum + group.totalDuration, 0)
                        )}
                      </span>
                    </div>
                  </TableCell>
