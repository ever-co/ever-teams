	...
	return (
		<Popover className={'relative '}>
			<div className="w-full flex ">
				...
				<Popover.Button
					className={cn('border relative grow rounded-lg w-full text-xs px-3 py-2', searchEnabled && ' pl-9')}
					{...(searchEnabled
						? {
			...
			<Popover.Panel className={'absolute w-full mt-1 z-20 bg-white rounded shadow-md'}>
				<ul className="w-full flex flex-col gap-[.125rem] p-[.125rem]">
					{options
						?.filter((option) => String(option.value).includes(searchTerm))
						?.map((option) => {
