...
export function TasksDataTable<TData, TValue>({ columns, data, className }: Readonly<DataTableProps<TData, TValue>>) {
	...
	return (
		<>
			<TeamTasksBanner data={data} table={table} />
				...
				<Table className="w-full">
					...
					<TableBody className="w-full">
						{table.getRowModel().rows.length ? (
							...
							<TableRow className="flex items-center justify-center w-full h-fit">
								<TableCell
									colSpan={columns.length}
									className="flex flex-col items-center justify-center flex-auto w-full h-24 p-4 text-center md:p-5"
								>
									<svg
