...
const Indicators = styled.div<{ bound?: 'row' | 'column' | undefined }>`
  position: absolute;
  top: 0;
  left: 0;
  width: 100%;
...
export const Resizer = ({ propKey, children, ...props }: any) => {
  ...
  const updateInternalDimensionsInPx = useCallback(() => {
    const { width: nodeWidth, height: nodeHeight }: any = nodeDimensions.current;

    if (resizable.current && resizable.current.resizable && resizable.current.resizable.parentElement) {
      const width = percentToPx(
        nodeWidth,
        resizable.current && // deepscan-disable-line
  ...
  return (
    <Resizable
      ...
      ref={(ref) => {
        if (ref && resizable.current !== ref) {
          (resizable as any).current = ref;
          if (resizable.current && resizable.current.resizable) { // deepscan-disable-line
