...
import { useCallbackRef } from '@ever-teams/atoms';
...
export const AuthCodeInputField = forwardRef<AuthCodeRef, AuthCodeProps>(
	(
		...
		if (!allowedCharactersValues.some((value) => value === allowedCharacters)) {
			throw new Error('Invalid value for allowed characters. Use alpha, numeric, or alphanumeric');
		}
		...
		const inputs = Array.from({ length }, (_, i) => {
			...
			const style = hintType // deepscan-disable-line UNUSED_DECL
				? {
						transitionDuration: `${i + 10}90ms`,
						transitionTimingFunction: 'ease-in-out',
						borderColor: hintColor[hintType]
					}
				: undefined;
			...
			return (
				<InputField
					...
					maxLength={1}
					className={cn('transition-[border-color]', inputClassName)}
