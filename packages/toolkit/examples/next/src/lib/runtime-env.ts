 ...
 * Same contract as apps/web/env-config.ts `readRuntimeEnv`, minus the browser payload: these demos
 * read exactly one value, inside the root layout's subtree, so a prop replaces the inline <script>.
 */
